...
class BaseModel(PrepareData):
    ...
    def run_croos_validation(self):
        ...
        scores = model_selection.cross_val_score(self.clf, features, labels, cv=cv,
                                                 scoring=mean_absolute_percentage_error_scoring, n_jobs=-1)
        print("cross validation scores: means, {}, std, {}, details,{}".format(np.absolute(scores.mean()), scores.std(),
                                                                               np.absolute(scores)))
